...
cppcheck -iclang-format-report.txt -iclang-tidy-report.txt --enable=all --library=qt.cfg --std=c++20 --language=c++ --output-file=cppcheck-report.txt --inline-suppr ./files/*.{cpp,c,hpp} --enable=all --suppress=missingInclude --suppress=missingIncludeSystem --suppress=unusedFunction --suppress=unknownMacro
# flawfinder
flawfinder --version
...
./run-clang-format.py --clang-format-executable="clang-format" --style="{BasedOnStyle: Microsoft, UseTab: Always, ColumnLimit: 180, NamespaceIndentation: All, Language: Cpp, SortIncludes: false}" ./files/*.{cpp,h,c,hpp} > clang-format-report.txt
